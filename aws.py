...
def make_vpc(vpc_config_name):
    ...
    connections = connect(vpc_region)

    ec2_conn = connections.ec2
    vpc_conn = connections.vpc
    s3_conn = connections.s3
...
def get_or_create_route_table(vpc_conn, vpc, route_name, internet_gateway):
    ...
    vpc_conn.create_route(route_table.id, ANYWHERE, internet_gateway.id)
    return route_table
...
def get_or_create_bastion_host(ec2_conn, s3_conn, vpc_config, bastion_host_name, vpc_id, subnet):
    ...
    security_group = get_or_create_vpc_security_group(ec2_conn, vpc_config, vpc_id)

    for reservation in fetch_running_reservations(ec2_conn, bastion_host_name):
        for instance in reservation.instances:
            public_ip = associate_elastic_ip(ec2_conn, instance)
...
def associate_elastic_ip(ec2_conn, instance):
    if instance.ip_address:
        return instance.ip_address
    elastic_ip = get_or_create_elastic_ip(ec2_conn)
    ec2_conn.associate_address(instance_id=instance.id, allocation_id=elastic_ip.allocation_id)
...
def get_or_create_elastic_ip(ec2_conn):
    ...
    return ec2_conn.allocate_address('vpc')
...
def get_or_create_vpc_security_group(ec2_conn, vpc_config, vpc_id):
    ...
    for security_group in ec2_conn.get_all_security_groups(filters=filters):
...
def allow_https_egress(ec2_conn, security_group, destination):
...
def delete_vpc(vpc_config_name):
    ...
    for vpc in filter_by_name(vpc_conn.get_all_vpcs, vpc_name):
        ...
        for reservation in ec2_conn.get_all_instances(filters=filters):
            for instance in reservation.instances:
        ...
        security_groups = [gp for gp in ec2_conn.get_all_security_groups() if gp.vpc_id == vpc.id]
        ...
        for route_table in vpc_conn.get_all_route_tables(filters=filters):
        ...
        for internet_gateway in filter_by_name(vpc_conn.get_all_internet_gateways, vpc_name):
            ...
            vpc_conn.detach_internet_gateway(internet_gateway.id, vpc.id)
...
def clear_all_permissions(ec2_conn, security_group):
    revoke_ingress_permissions(ec2_conn, security_group)
    revoke_egress_permissions(ec2_conn, security_group)
